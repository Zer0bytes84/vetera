...
export async function hashPassword(password: string): Promise<string> {
    // TODO: Appeler une commande Tauri Rust pour bcrypt
    // Pour l'instant: hash basique (NE PAS UTILISER EN PROD)
...
export async function register(data: RegisterData): Promise<AuthUser> {
    if (!isTauriRuntime()) {
        const existing = findBrowserRow<BrowserUserRecord>("users", (entry) => entry.email === data.email);
        if (existing) {
    ...
    await db.execute(
        ...
        [userId, data.email, passwordHash, data.displayName, 'stagiaire', 'active']
    );
...
export async function logout(): Promise<void> {
    ...
    if (token) {
        if (!isTauriRuntime()) {
            clearBrowserRows("sessions", (session) => (session as BrowserSessionRecord).token === token);
...
export async function getCurrentUser(): Promise<AuthUser | null> {
    ...
    if (!isTauriRuntime()) {
        ...
        const user = findBrowserRow<BrowserUserRecord>("users", (entry) => entry.id === session.userId);
        if (!user) {
            return null;
        }
...
export async function createInitialAdmin(data: RegisterData): Promise<AuthUser> {
    if (!isTauriRuntime()) {
        ...
        const createdUser = findBrowserRow<BrowserUserRecord>("users", (entry) => entry.email === data.email);
        return {
            id: createdUser?.id ?? "",
