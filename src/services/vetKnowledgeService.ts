...
function calculateRelevance(
  ...
  const queryTerms = normalizeString(query)
    .split(/\s+/)
    .filter((t) => t.length > 2);
  ...
  for (const term of queryTerms) {
    if (normalizedText.includes(term)) {
      ...
      if (new RegExp(`\\b${escapeRegExp(term)}\\b`).test(normalizedText)) {
        score += 0.3;
      }
    }
  }
...
function objectToSearchableText(obj: unknown): string {
  ...
  if (Array.isArray(obj)) {
    return obj.map(objectToSearchableText).join(" ");
  }
...
class VetKnowledgeService {
  ...
  constructor() {
    ...
    const baseMeds =
      (medicationsData as { medications: Medication[] }).medications || [];
    const extendedMeds =
      (
        medicationsExtendedData as unknown as {
          additionalMedications: Medication[];
        }
      ).additionalMedications || [];
    this.medications = [...baseMeds, ...extendedMeds];

    this.diseases =
      (diseasesData as unknown as { diseases: Disease[] }).diseases || [];
    this.species = (speciesData as { species: Species[] }).species || [];
    this.emergencies =
      (emergenciesData as { emergencies: Emergency[] }).emergencies || [];
    this.referenceData = referenceData;

    console.log(
      `[VetKnowledge] Loaded: ${this.medications.length} medications, ${this.diseases.length} diseases, ${this.species.length} species, ${this.emergencies.length} emergencies`
    );
  }
    ...
    return (
      this.medications.find(
        (med) =>
          ...
          med.nomCommercial?.some(
            (nc) => normalizeString(nc) === normalizedQuery
          )
      ) || null
    );
  }
  ...
  getDosage(medicationName: string, species: string): string | null {
    ...
    for (const [sp, dosage] of Object.entries(med.posologies)) {
      if (normalizeString(sp).includes(normalizedSpecies)) {
        ...
        if (dosage.voie) {
          result += `- Voie: ${dosage.voie}\n`;
        }
        if (dosage.duree) {
          result += `- Durée: ${dosage.duree}\n`;
        }
        if (dosage.attention) {
          result += `⚠️ ${dosage.attention}\n`;
        }
        return result;
      }
    }
...
- Posologies: ${Object.entries(med.posologies)
          .map(([sp, d]) => `${sp}: ${d.dose} ${d.frequence}`)
          .join("; ")}
  ...
  getLabValues(
    ...
    if (category) {
      ...
      if (cat && typeof cat === "object") {
        return (
          ((cat as Record<string, unknown>)[normalizedSpecies] as Record<
            string,
            unknown
          >) || null
        );
    ...
    for (const [catName, catData] of Object.entries(labValues)) {
      if (typeof catData === "object" && catData !== null) {
        const speciesData = (catData as Record<string, unknown>)[
          normalizedSpecies
        ];
        if (speciesData) {
