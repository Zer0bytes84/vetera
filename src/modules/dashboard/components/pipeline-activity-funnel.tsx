...
export function PipelineActivityFunnel({
  ...
  const avgSpark = sparklineData.length > 0 ? totalSpark / sparklineData.length : 0;
  ...
  return (
    <Card className="dashboard-luxe-card group relative flex h-full flex-col overflow-hidden p-6 shadow-none !border-zinc-200 dark:!border-white/10 transition-[transform,shadow] duration-300 hover:-translate-y-0.5 hover:shadow-md hover:shadow-zinc-950/5 dark:hover:shadow-black/20">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -right-12 -top-12 h-44 w-44 rounded-full bg-teal-500/10 blur-3xl dark:bg-teal-500/5" />
        <div className="absolute -left-12 -bottom-12 h-40 w-40 rounded-full bg-emerald-500/10 blur-3xl dark:bg-emerald-500/5" />
      </div>
      ...
      <div className="relative z-10 flex h-full flex-col">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-teal-500 to-emerald-400 dark:from-teal-400 dark:to-emerald-300 font-sans">
                Flux &amp; Cadence
              </span>
            </div>
        ...
        <div className="mt-5 space-y-3 border-t border-zinc-950/10 pt-4 dark:border-white/10">
          <div className="flex items-center justify-between">
            <h4 className="flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest text-muted-foreground">
            ...
            <div className="flex items-center gap-2">
              <span
                className={cn(
                  "flex items-center gap-0.5 rounded-full px-1.5 py-0 text-[10px] font-bold tabular-nums",
                  trendUp
          ...
          <div className="grid grid-cols-14 gap-1">
            {sparklineData.map((day, idx) => {
              const intensity = day.val === 0 ? 0 : Math.max(0.15, day.val / maxSparklineVal);
              const isMax = day.val === maxSparklineVal && day.val > 0;
              return (
                <div key={idx} className="group/heat relative">
                  {/* Heat cell */}
                  <div
                    className={cn(
                      "relative h-12 overflow-hidden rounded-md transition-all duration-300 hover:scale-105 cursor-pointer",
                      day.val === 0
                  ...
                  <p className="mt-1 text-center text-[9px] font-bold uppercase text-muted-foreground/70 tabular-nums">
                    {day.dayLetter}
                  </p>
                  {/* Tooltip */}
                  <div className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-max -translate-x-1/2 opacity-0 transition-opacity duration-200 group-hover/heat:opacity-100">
                    <div className="rounded-md bg-zinc-900 px-2 py-1 text-[10px] font-medium text-white shadow-xl dark:bg-white dark:text-zinc-900">
                      {day.label} :{" "}
                      <span className="font-bold">{day.val}</span> rdv
                    </div>
                  </div>
                </div>
              );
