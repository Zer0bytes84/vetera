...
function categoryIcon(category?: string) {
  ...
  if (/(vaccin|sérum|serum|injection)/.test(c)) {
    return Syringe;
  }
  if (/(médicament|medicament|comprim|pilule|pill)/.test(c)) {
    return Pill;
  }
  return Package2;
}
...
export function StockAlertsWidget({ onNavigate }: StockAlertsWidgetProps) {
  ...
  const stats = React.useMemo(() => {
    ...
    const outOfStock = stockAlerts.filter(
  ...
  return (
    <Card className="dashboard-luxe-card group !border-zinc-200 dark:!border-white/10 relative flex h-full flex-col overflow-hidden shadow-none transition-[transform,shadow] duration-300 hover:-translate-y-0.5 hover:shadow-md hover:shadow-zinc-950/5 dark:hover:shadow-black/20">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -top-12 -right-12 h-44 w-44 rounded-full bg-rose-500/10 blur-3xl dark:bg-rose-500/5" />
        <div className="absolute -bottom-12 -left-10 h-40 w-40 rounded-full bg-orange-500/10 blur-3xl dark:bg-orange-500/5" />
      </div>
      ...
      <div className="relative z-10 flex h-full flex-col p-6">
        ...
        <div className="flex items-start justify-between gap-4 border-zinc-950/10 border-b pb-4 dark:border-white/10">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-gradient-to-r from-rose-500 to-orange-400 bg-clip-text font-extrabold font-sans text-[10px] text-transparent uppercase tracking-widest dark:from-rose-400 dark:to-orange-300">
          ...
          {stockAlerts.length > 0 ? (
            ...
            <div className="flex shrink-0 items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 font-bold text-emerald-700 text-xs dark:bg-emerald-500/5 dark:text-emerald-300">
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-3 text-[10px] text-muted-foreground">
            ...
            <LegendDot
              color="bg-rose-500"
              label={`${stats.outOfStock} rupture`}
            />
          </div>
        </div>
          ...
          {stockAlerts.length === 0 ? (
            ...
            stockAlerts.slice(0, 4).map((product) => {
              ...
              return (
                <div
                  className={cn(
                    "group/row relative rounded-2xl border border-transparent p-3 transition-all hover:border-zinc-200/60 hover:bg-white/60 hover:shadow-xs dark:hover:border-white/10 dark:hover:bg-white/[0.04]",
                    isOut &&
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span
                          className={cn(
                            "shrink-0 rounded-full border px-2 py-0 font-bold text-[10px] tracking-wide",
                            isOut
                              ? "border-rose-500/30 bg-rose-500/15 text-rose-700 shadow-[0_0_8px_rgba(244,63,94,0.2)] dark:text-rose-300"
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <div className="relative h-1.5 flex-1 overflow-hidden rounded-full bg-zinc-950/5 dark:bg-white/5">
                      <div
                        ...
                        style={{
                          width: `${Math.max(isOut ? 100 : 8, ratio * 100)}%`,
                        }}
                      />
                    </div>
                    <span
