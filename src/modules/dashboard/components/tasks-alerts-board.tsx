  ...
  return (
    <Card className="dashboard-luxe-card group !border-zinc-200 dark:!border-white/10 relative flex h-full flex-col overflow-hidden shadow-none transition-[transform,shadow] duration-300 hover:-translate-y-0.5 hover:shadow-md hover:shadow-zinc-950/5 dark:hover:shadow-black/20">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -top-12 -left-12 h-44 w-44 rounded-full bg-amber-500/10 blur-3xl dark:bg-amber-500/5" />
      ...
      <div className="relative z-10 flex h-full flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative flex h-14 w-14 shrink-0 items-center justify-center">
              ...
              >
                ...
                />
                <defs>
                  <linearGradient id="tasks-grad" x1="0" x2="1" y1="0" y2="1">
                    <stop offset="0%" stopColor="#f59e0b" />
                    <stop offset="100%" stopColor="#f43f5e" />
                  </linearGradient>
                </defs>
              </svg>
              <span className="font-bold font-display text-foreground text-sm tabular-nums">
                {Math.round(stats.ratio * 100)}%
              </span>
            </div>

            <div className="min-w-0 flex-1">
              <span className="bg-gradient-to-r from-amber-500 to-rose-400 bg-clip-text font-extrabold font-sans text-[10px] text-transparent uppercase tracking-widest dark:from-amber-400 dark:to-rose-300">
                Urgences &amp; Actions
              </span>
              <h3 className="mt-1 truncate font-display font-semibold text-foreground text-lg tracking-tight">
                Tableau des Tâches
              </h3>
            ...
            {stats.urgent > 0 && (
              <div className="flex shrink-0 items-center gap-1.5 rounded-full border border-rose-500/20 bg-rose-500/10 px-2.5 py-1 font-bold text-rose-700 text-xs shadow-[0_0_10px_rgba(244,63,94,0.15)] dark:bg-rose-500/5 dark:text-rose-300">
          ...
          <div className="mt-3 flex items-center gap-1.5 overflow-x-auto pb-0.5">
            {(["all", "high", "medium", "low"] as FilterPriority[]).map((f) => {
              ...
              return (
                <button
                  className={cn(
                    "shrink-0 rounded-full border px-2.5 py-0.5 font-bold text-[10px] transition-all",
                    active
        ...
        {/* ── List ──────────────────────────────────────────── */}
        <div className="max-h-[290px] flex-1 overflow-y-auto px-2 py-2">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
              ...
              <p className="text-muted-foreground text-xs">
                Aucune tâche{" "}
                {filter === "all" ? "en attente" : "dans ce filtre"}
              </p>
            </div>
          ) : (
              ...
              {filtered.slice(0, 5).map((task) => {
                ...
                return (
                  <li key={task.id}>
                    <div
                      className={cn(
                        "group/row relative flex items-center gap-3 rounded-2xl border border-transparent px-3 py-2.5 pl-4 transition-all duration-300 hover:border-zinc-200/60 hover:bg-white/60 hover:shadow-xs dark:hover:border-white/10 dark:hover:bg-white/[0.04]",
                        isHigh &&
                          "border-rose-500/10 bg-rose-500/[0.025] dark:border-rose-400/10 dark:bg-rose-400/[0.02]"
                      )}
                    >
                      {/* Priority accent bar */}
                      <span
                        className={cn(
                          "absolute top-3 bottom-3 left-0.5 w-[3px] rounded-full opacity-70 transition-all group-hover/row:opacity-100",
                          meta.bar
                        )}
                      />
                      ...
                      <button
                        ...
                        className={cn(
                          "flex h-5 w-5 shrink-0 cursor-pointer items-center justify-center rounded-md border-2 text-white ring-2 ring-transparent transition-all active:scale-90",
                          task.status === "done"
                      ...
                      <div className="min-w-0 flex-1">
                        <p
                          className={cn(
                            "truncate font-semibold text-foreground text-sm leading-tight",
                            task.status === "done" &&
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          <span
                            className={cn(
                              "inline-flex items-center gap-1 rounded-full border px-1.5 py-0 font-bold text-[9px] uppercase tracking-wide",
                              meta.tone
                            )}
                          >
