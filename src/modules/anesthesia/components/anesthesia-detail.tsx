...
export function AnesthesiaDetail({
  ...
  const handleStatusChange = async (next: AnesthesiaStatus) => {
    ...
    if (next === "in_progress" && !sheet.startedAt) {
      updates.startedAt = now;
    }
    if ((next === "completed" || next === "cancelled") && !sheet.endedAt) {
      updates.endedAt = now;
    }
            ...
            {sheet.premedication || sheet.induction || sheet.maintenance ? (
              <div className="grid gap-3 sm:grid-cols-2">
                {sheet.premedication ? (
                  <InfoBlock
                    label={t(
                      "modules.anesthesia.fields.premedication",
                      "Prémédication"
                    )}
                ...
                {sheet.induction ? (
                  <InfoBlock
                    label={t(
                      "modules.anesthesia.fields.induction",
                      "Induction"
                    )}
                ...
                {sheet.maintenance ? (
                  <InfoBlock
                    label={t(
                      "modules.anesthesia.fields.maintenance",
                      "Maintenance"
                    )}
        ...
        <Card className="h-fit">
          ...
          <CardContent className="p-0">
            {drugs.length === 0 ? (
              <Empty className="m-6 border border-dashed">
                <EmptyHeader>
                  <EmptyTitle>
                    {t(
                      "modules.anesthesia.drugLog.noEntries",
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end gap-0.5 text-right">
                      ...
                      <span className="text-[11px] text-muted-foreground">
                        {new Date(d.administeredAt).toLocaleTimeString(
                          "fr-FR",
                          {
                            hour: "2-digit",
                            minute: "2-digit",
            ...
            <div className="flex flex-wrap items-center gap-2 border-border/40 border-t bg-muted/20 px-6 py-2 text-[11px] text-muted-foreground">
              ...
              {(
                ["premed", "induction", "maintenance", "recovery"] as const
              ).map((p) => (
                <Badge
                  className="border-border/40 bg-background"
                  key={p}
          ...
          {monitoring.length === 0 ? (
            <Empty className="m-6 border border-dashed">
              <EmptyHeader>
                <EmptyTitle>
                  {t(
                    "modules.anesthesia.monitoring.noEntries",
                    "Aucun point de monitoring"
                  )}
                </EmptyTitle>
              </EmptyHeader>
            </Empty>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {monitoring.map((m) => (
                    <tr className="border-border/30 border-b" key={m.id}>
