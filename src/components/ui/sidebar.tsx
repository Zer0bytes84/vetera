...
const SIDEBAR_EXPAND_THRESHOLD = 232;
...
function Sidebar({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative w-(--sidebar-width) bg-transparent transition-[width] duration-[240ms] ease-[var(--ease-out)] group-data-[resizing=true]/sidebar-wrapper:duration-0",
      ...
      <div
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-[240ms] ease-[var(--ease-out)] data-[side=right]:right-0 data-[side=left]:left-0 group-data-[resizing=true]/sidebar-wrapper:duration-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] md:flex",
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <HugeiconsIcon
        className="size-[18px] text-sidebar-foreground/70 transition-colors hover:text-sidebar-foreground rtl:rotate-180"
        icon={LayoutLeftIcon}
        strokeWidth={1.5}
      />
...
function SidebarRail({
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    if (!dragState.current.dragged && Math.abs(delta) >= 6) {
    ...
    if (dragState.current.dragged) {
      const pointerWidth =
        dragState.current.side === "right"
          ? window.innerWidth - event.clientX
          : event.clientX;

      if (pointerWidth <= SIDEBAR_COLLAPSE_THRESHOLD) {
        if (!dragState.current.collapsedByDrag) {
          dragState.current.collapsedByDrag = true;
          setOpen(false);
        }
        return;
      }
      ...
      if (dragState.current.collapsedByDrag) {
        if (pointerWidth < SIDEBAR_EXPAND_THRESHOLD) {
          return;
        }
        dragState.current.collapsedByDrag = false;
        setOpen(true);
      }

      setSidebarWidth(pointerWidth);
    }
  };
  ...
  return (
    <button
      aria-label="Redimensionner ou réduire la barre latérale"
      className={cn(
        "group/rail absolute inset-y-0 z-50 hidden w-3 touch-none select-none items-center justify-center outline-none group-data-[side=left]:-right-1.5 group-data-[side=right]:-left-1.5 sm:flex",
        "cursor-col-resize focus-visible:after:opacity-100 focus-visible:after:ring-2 focus-visible:after:ring-primary/30",
        "after:h-12 after:w-1 after:rounded-full after:border after:border-white/70 after:bg-zinc-300/80 after:opacity-0 after:shadow-[0_2px_10px_rgba(15,23,42,0.16)] after:backdrop-blur-md after:transition-[opacity,background-color,height] after:duration-200 hover:after:h-16 hover:after:bg-zinc-400/80 hover:after:opacity-100 active:after:h-20 active:after:bg-primary dark:after:border-white/10 dark:after:bg-zinc-600/80 dark:hover:after:bg-zinc-500/90",
        "group-data-[resizing=true]/sidebar-wrapper:after:h-20 group-data-[resizing=true]/sidebar-wrapper:after:bg-primary group-data-[resizing=true]/sidebar-wrapper:after:opacity-100",
        className
      )}
      data-sidebar="rail"
      ...
      title="Glisser pour redimensionner jusqu’aux icônes · Cliquer pour réduire · Double-cliquer pour réinitialiser"
      {...props}
    />
  );
}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full cursor-pointer items-center gap-2 overflow-hidden rounded-md p-2 text-start text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding,margin] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pe-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:mx-auto group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&_svg]:size-5 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
